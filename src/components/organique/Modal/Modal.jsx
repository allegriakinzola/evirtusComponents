...
const Modal = ({
  ...
  const p = fullWith ? "" : "p-4";
  return (
    <div
      id="small-modal"
      tabIndex="0"
      className={`${state} bg-backdrop overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 w-full inset-0 h-full`}
    >
      <div
        className={`${className} relative ${p} w-full mx-auto h-full flex justify-center items-${position}`}
      >
        <div
          className={`${contentContainerClassName} relative bg-white ${
            fullWith ? "" : "rounded-lg"
          } shadow w-full`}
        >
